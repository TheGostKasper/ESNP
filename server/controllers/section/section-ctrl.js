...
module.exports = function (app) {
    app.post('/api/section', (req, res) => {
        ...
        if (req.body.length > 1)
            req.body.forEach(function (element) {
                addSectionAsync(element, res).then((data) => { }).catch(err => console.log(err));
            }, this);
        else {
    ...
    async function getLastAsync(body) {
        try {
            return await Section.findOne(body).exec();
        } catch (error) {
            res.send({ data: null, err: error });
        }
    }
    ...
    async function getSectionAsync(option) {
        try {
            return await Section.find(option).populate('Employee').exec();
        } catch (error) {
            return error;
        }
    }
